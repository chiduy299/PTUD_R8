...
namespace ptud_project.Controllers
{
    ...
    public class StoreController : ControllerBase
    {
        ...
        [HttpPut("change_status/{id_store}")]
        ...
        [HttpPut("confirm_order")]
        public IActionResult ConfirmOrderById([FromBody] StoreRequestOrderModel request)
        {
            MongoClient dbClient = new MongoClient(_configuration.GetConnectionString("PtudhtttDB"));
            var order = dbClient.GetDatabase("ptudhttt").GetCollection<Order>("Orders").AsQueryable().Where(x => x.store_id == request.store_id && x.order_id == request.order_id).FirstOrDefault();
            if (order == null)
            {
                return Ok(new
                {
        ...
        [HttpPut("request_shipping")]
        public IActionResult ChangeShippingStatusOrderById([FromBody] StoreRequestOrderModel request)
        {
            MongoClient dbClient = new MongoClient(_configuration.GetConnectionString("PtudhtttDB"));
            var order = dbClient.GetDatabase("ptudhttt").GetCollection<Order>("Orders").AsQueryable().Where(x => x.store_id == request.store_id && x.order_id == request.order_id).FirstOrDefault();
            if (order == null)
