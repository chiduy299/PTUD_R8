...
namespace ptud_project.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        public IActionResult GetOrderByCustomer([FromQuery] string customer_id)
        {
            MongoClient dbClient = new MongoClient(_configuration.GetConnectionString("PtudhtttDB"));
            var orders = dbClient.GetDatabase("ptudhttt").GetCollection<Order>("Orders").AsQueryable().Where(x => x.customer_id == customer_id).ToList();
            return Ok(new
            {
                code = 0,
                message = "Success",
        ...
        public IActionResult GetDeliveryHistoryByShipper([FromQuery] string shipper_id)
        {
            try
            {
                MongoClient dbClient = new MongoClient(_configuration.GetConnectionString("PtudhtttDB"));
                var orders = dbClient.GetDatabase("ptudhttt").GetCollection<Order>("Orders").AsQueryable().Where(x => x.shipper_id == shipper_id).ToList();
        ...
        public async Task<IActionResult> CreateOrderAsync([FromBody] CreateOrderRequest request)
        {
            try
            {
                ...
                return Ok(new { code = 0, message = "Success", payload = order });
        ...
        [HttpPost("purchase")]
        public async Task<IActionResult> PurchaseOrder([FromBody] RequestPurchase request)
        {
            try
            {
                MongoClient dbClient = new MongoClient(_configuration.GetConnectionString("PtudhtttDB"));
                var order = dbClient.GetDatabase("ptudhttt").GetCollection<Order>("Orders").AsQueryable().Where(x => x.order_id == request.order_id).FirstOrDefault();
                if (order == null)
                {
                    return Ok(new { code = 0, message = "Not existing data"});
                }
                ...
                var myjson = new MomoCreatePaymentRequest
                {
                    partnerCode = partner_code,
                    requestId = order.order_id,
                    amount = (Int64)order.total_amount,
                    orderId = order.order_id,
                    orderInfo = "Don hang cua R8",
                    redirectUrl = return_url,
                    ipnUrl = ipn_url,
                ...
                using (var client = new HttpClient())
                {
                    ...
                    MomoCreatePaymentResponse resp = JsonSerializer.Deserialize<MomoCreatePaymentResponse>(result);
                    return Ok(new { code = 0, message = "Success", payload = resp });
                }
            }
            catch
            {
