...
namespace ptud_project.Controllers
{
    ...
    public class ShipperController : Controller
    {
        ...
        [HttpPut("receive_order")]
        public IActionResult ReceiverOrder([FromBody] ShipperRequestOrderModel request)
        {
            try
            {
                //validate
                // kiem tra sdt dang ky da ton tai chua?
                MongoClient dbClient = new MongoClient(_configuration.GetConnectionString("PtudhtttDB"));
                var shipper = dbClient.GetDatabase("ptudhttt").GetCollection<Shipper>("Shippers").AsQueryable().Where(x => x.id == request.shipper_id).FirstOrDefault();
                if (shipper == null)
                ...
                dbClient.GetDatabase("ptudhttt").GetCollection<Order>("Orders").FindOneAndReplace(x => x.order_id == request.order_id, order);
                return Ok(new
                {
                    code = 0,
        ...
        [HttpPut("success_order")]
        public IActionResult ChangeSuccessStatusOrderById([FromBody] ShipperRequestOrderModel request)
        {
            MongoClient dbClient = new MongoClient(_configuration.GetConnectionString("PtudhtttDB"));
            var order = dbClient.GetDatabase("ptudhttt").GetCollection<Order>("Orders").AsQueryable().Where(x => x.order_id == request.order_id && x.shipper_id == request.shipper_id).FirstOrDefault();
            if (order == null)
            {
                return Ok(new
                {
            ...
            order.status = 5;
            order.updated_at = helper.now_to_epoch_time();
            dbClient.GetDatabase("ptudhttt").GetCollection<Order>("Orders").ReplaceOne(x => x.order_id == request.order_id, order);
